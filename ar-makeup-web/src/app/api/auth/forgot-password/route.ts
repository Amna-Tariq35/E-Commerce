...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ Seedha Supabase ko command di ki is email par password reset link bhej do
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${origin}/auth/reset-password`,
    });

    // Agar Supabase ne koi error diya (jaise email invalid hai), toh ab '400' status jayega
    if (error) {
      console.error("[forgot-password API] Supabase Error:", error.message);
      return NextResponse.json(
        { error: error.message },
        { status: 400 } 
      );
